...
class Custom_GPY(Kern):
    ...
    def K(self, X, X2):
        ...
        if X2 is None:  # Symmetric case
            ...
            self.E_symm = self.emb_scale * np.array(np.matmul(X[:, 1:], X[:, 1:].T), dtype=np.float32) # XX^T is correct
    ...
    def update_gradients_full(self, dL_dK, X, X2):
        ...
        if X2 is None:
            ...
            for i in range(n):
                dlambda_diag[i] = part_deriv(X[i, 0], X[i, 0], self.lamb*1)# * 1)
            ...
            dE = np.array(np.matmul(X[:, 1:], X[:, 1:].T), dtype=np.float32)
            ...
            for i in range(n):
                for j in range(n2):
                    dlambda[i, j] = part_deriv(X[i, 0], X2[j, 0], self.lamb*1)# * 1)

            # Compute dL/demb_scale
            dE = np.array(np.matmul(X[:, 1:], X2[:, 1:].T), dtype=np.float32)
            self.emb_scale.gradient = np.sum(dE * dL_dK)
...
def initialize_kernel(num_col, internal_seed):
    ...
    return Custom_GPY(input_dim=num_col, lamb=0.7012455615069223, e=0.8847221539035289)