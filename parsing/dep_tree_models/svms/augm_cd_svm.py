...
def CustomKernel(X, X2=None, lamb=1, emb_scale=1):
    ...
    if X2 is None:  # Symmetric case
        ...
        E_symm = emb_scale * np.array(np.matmul(X[:, 1:], X[:, 1:].T), dtype=np.float32) # XX^T is correct
        ...
        # Calculate the embeddings matrix
        E_asymm = emb_scale * np.array(np.matmul(X[:, 1:], X2[:, 1:].T), dtype=np.float32)
...
def fit_and_predict_svm(X_train, X_test, y_train, params):
    ...
    gram_test = CustomKernel(X=X_test, X2=X_train, lamb=0.7012455615069223, emb_scale=0.8847221539035289)

    # Fit the model
    model = SVC(C=params["C"], kernel="precomputed", random_state=42)
