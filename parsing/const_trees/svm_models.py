...
from helper_functions import extract_sentences, prepare_dataset, evaluate_classifier
...
def main():
    ...
    for shuffle_i in range(SHUFFLES):
        ...
        param_grid = {'C': np.logspace(-5, 0, num=4, endpoint=True), 'lamb': np.linspace(1e-5, 1, num=4, endpoint=True), 'e': np.logspace(-5, 0, num=6, endpoint=True)}
        ...
        params_avg_res = Parallel(n_jobs=48)(delayed(run_cv_on_hyperparameter_combination)(params=params, data=data, folds=folds, num_cands=num_cands) for params in param_comb)
